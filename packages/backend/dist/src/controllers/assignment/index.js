...
import { createAssignment, createSubmission, findAssignmentById, 
// findAssignmentsByClass,
findEnrollment, findSubmissionByAssignmentAndStudent,
// findSubmissionsByAssignment,
// updateSubmissionGrade,
 } from "@backend-db/utils";
import { and, asc, desc, eq, sql } from "drizzle-orm";
import { sendBadRequest, sendForbidden, sendInternalServerError, sendNotFound, sendSuccess,
// sendUnauthorized,
...
export async function createAssignmentController(req, res) {
    try {
        const { title, description, classId, dueDate, points = 100, assignmentType = "assignment", attachments = [], settings, } = req.body;
...
export async function getClassAssignments(req, res) {
    try {
        ...
        if (status) {
            whereClauses.push(eq(assignments.status, status));
        }
...
export async function getAssignmentSubmissions(req, res) {
    try {
        ...
        const canViewSubmissions = currentUser.role === "admin"
            || classRecord[0].ownerId === currentUser.id
            || assignment.createdBy === currentUser.id;
...
export async function gradeSubmission(req, res) {
    try {
        ...
        const canGrade = currentUser.role === "admin"
            || classRecord[0].ownerId === currentUser.id
            || assignment.createdBy === currentUser.id;
        if (!canGrade) {
