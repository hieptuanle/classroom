...
import { createClass, createEnrollment, findClassByCode, findClassById, findClassEnrollments, findEnrollment, updateClassInviteCode, } from "@backend-db/utils";
import { and, asc, eq, or, sql } from "drizzle-orm";
import { sendBadRequest, sendForbidden, sendInternalServerError, sendNotFound, sendSuccess, } from "@backend/helpers/response";
...
export async function getMyClasses(req, res) {
    try {
        ...
        // Add search filter
        if (search) {
            whereClauses.push(or(sql `${classes.name} ILIKE ${`%${search}%`}`, sql `${classes.description} ILIKE ${`%${search}%`}`, sql `${classes.classCode} ILIKE ${`%${search}%`}`));
        }
        // Add pagination
...
export async function getClassEnrollments(req, res) {
    try {
        const { id } = req.params;
        const currentUser = req.currentUser;
        const classRecord = await findClassById(id);
...
export async function removeUserFromClass(req, res) {
    try {
